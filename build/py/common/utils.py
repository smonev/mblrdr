...
def CreateFirstTimeUser(user, demo):
    username = CreateUsername(user.email())

    blogs = [{"url": "http://feeds.feedburner.com/TechCrunch", "title": "TechCrunch"}, {"url": "http://feeds.feedburner.com/ommalik", "title": "GigaOM"}, {"url": "http://feeds.feedburner.com/readwriteweb", "title": "ReadWrite"}, {"url": "http://feeds.feedburner.com/typepad/alleyinsider/silicon_alley_insider", "title": "SAI"}, {"url":
                                                                                                                                                                                                                                                                                                                                         "http://feeds.arstechnica.com/arstechnica/index", "title": "Ars Technica"}, {"url": "http://feeds.paidcontent.org/pcorg", "title": "paidContent"}, {"url": "http://www.engadget.com/rss.xml", "title": "Engadget RSS Feed"}, {"url": "http://feeds.gawker.com/gizmodo/full", "title": "Gizmodo"}, {"url": "http://feeds.feedburner.com/TheBoyGeniusReport", "title": "BGR"}]
    jsonBlogList = '{"username":"' + username + \
        '", "bloglist":{"root":[{"url":"http://feeds.feedburner.com/TechCrunch","title":"TechCrunch"}], "1":[{"url":"http://feeds.feedburner.com/ommalik","title":"GigaOM"},{"url":"http://feeds.feedburner.com/readwriteweb","title":"ReadWrite"},{"url":"http://feeds.feedburner.com/typepad/alleyinsider/silicon_alley_insider","title":"SAI"},{"url":"http://feeds.arstechnica.com/arstechnica/index","title":"Ars Technica"},{"url":"http://feeds.paidcontent.org/pcorg","title":"paidContent"}],"2":[{"url":"http://www.engadget.com/rss.xml","title":"Engadget RSS Feed"},{"url":"http://feeds.gawker.com/gizmodo/full","title":"Gizmodo"}, {"url":"http://feeds.feedburner.com/TheBoyGeniusReport","title":"BGR"}]}, "userSettings": {"nightmode": false}}'
    ud = UserData(app_username=username, id=user.email(),
                  private_data=jsonBlogList, isActive=True, demo=demo)
    ...
    for feed in blogs:
        readDataAttr = 'readData__' + username + \
            '__' + str(feed['url']).translate(None, '.')

        rd = ReadData(app_username=username, feedUrl=feed[
                      'url'], readData='', readCount=0, id=readDataAttr)
        rd.put_async()
...
def GetFeedDataSettings(feed):
    ...
    if fds is None:
        ...
        if (ud is not None):
            ...
            rd = ReadData(app_username=app_username, feedUrl=feed,
                          readData='', readCount=0, id=readDataAttr)
            rd.put_async()
...
def AddSomeData(d, feed, items, feedDataSettings, createNew):
    ...
    # http://stackoverflow.com/questions/5022725/how-do-i-measure-the-memory-usage-of-an-object-in-python
    ...
    if createNew:
        ...
        newKeyName = str(hash(feed) * hash(feed)) + '_' + \
            str(feedDataSettings.feedDataCount)
        newFeedData = FeedData(
            url=feed, private_data=feedDataSettings.private_data, id=newKeyName)
        newFeedData.put()

        feedDataSettings.latest_item = newKeyName
        feedDataSettings.private_data = ''

    feedDataSettings.article_count = feedDataSettings.article_count + \
        newItemsCount

    logging.debug('[READCOUNT DEBUG] Increase article count of %s to %s, createNew: %s',
                  feed, feedDataSettings.article_count, createNew)
